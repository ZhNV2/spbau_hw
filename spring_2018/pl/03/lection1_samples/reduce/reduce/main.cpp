...
int main()
{
   ...
   try {
      ...
      queue.finish();
      cl::Kernel kernel_gmem(program, "gpu_reduce_gmem");
      cl::KernelFunctor reduce_gmem(kernel_gmem, queue, cl::NullRange, cl::NDRange(test_array_size), cl::NDRange(block_size));
      //cl::Event event = reduce_gmem(dev_input, dev_output);

      cl::Kernel kernel_lmem(program, "gpu_reduce_lmem");
      cl::KernelFunctor reduce_lmem(kernel_lmem, queue, cl::NullRange, cl::NDRange(test_array_size), cl::NDRange(block_size));
      cl::Event event = reduce_lmem(dev_input, dev_output, cl::__local(sizeof(int)* block_size));
